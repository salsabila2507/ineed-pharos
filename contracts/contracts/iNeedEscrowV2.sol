...
contract iNeedEscrowV2 {
    ...
    event TaskCreated(uint256 indexed taskId, address indexed creator, address indexed rewardAsset, uint256 rewardTotal, uint256 deadline);
    ...
    function createTask(
        ...
        emit TaskCreated(taskId, msg.sender, rewardAsset_, rewardTotal_, deadline_);
    }
    ...
    function deposit(uint256 taskId) external payable onlyCreator(taskId) {
        ...
        if (task.rewardAsset == address(0)) {
            require(msg.value == task.rewardTotal, "msg.value must equal rewardTotal");
        } else {
            require(msg.value == 0, "msg.value must be 0 for ERC20 deposit");
            IERC20 token = IERC20(task.rewardAsset);
            require(
                token.transferFrom(msg.sender, address(this), task.rewardTotal),
        ...
        escrow.totalAmount = task.rewardTotal;

        task.status = TaskStatus.Open;

        emit TaskFunded(taskId, task.rewardTotal);
    }
    ...
    function release(uint256 taskId) external onlyCreator(taskId) nonReentrant {
        ...
        if (task.rewardModel == RewardModel.Single) {
            ...
            _transferRewardAndEmit(taskId, task.rewardAsset, winnerAddresses[0], payoutPool, escrow);
        } else {
        ...
        if (fee > 0) {
            _transferRewardToTreasury(taskId, task.rewardAsset, task.feeTreasury, fee, escrow);
        }
    ...
    function refund(uint256 taskId) external onlyCreator(taskId) nonReentrant {
        ...
        _safeTransferReward(task.rewardAsset, task.creator, amount);
    ...
    function resolveDispute(
        ...
        if (ruling == DisputeRuling.InFavorOfParticipant) {
            ...
            if (netDistribution > 0) {
                _distributeProportionally(taskId, task.rewardAsset, recipients, amounts, totalGross, netDistribution, escrow);
            }
            ...
            if (remainder > 0) {
                _safeTransferReward(task.rewardAsset, task.creator, remainder);
                escrow.refundedAmount += remainder;
            }
        } else if (ruling == DisputeRuling.InFavorOfCreator) {
            _safeTransferReward(task.rewardAsset, task.creator, balance);
            ...
            if (netPayout > 0) {
                _distributeProportionally(taskId, task.rewardAsset, recipients, amounts, totalToParticipants, netPayout, escrow);
            }
            ...
            if (creatorAmount > 0) {
                _safeTransferReward(task.rewardAsset, task.creator, creatorAmount);
        ...
        if (action == AutoTimeoutAction.PayAll) {
            ...
            for (uint256 i = 0; i < subCount; i++) {
                ...
                uint256 amount = (i == subCount - 1)
                    ...
                    : rewardPerSubmitter;
                _safeTransferReward(task.rewardAsset, taskSubmissions[i].submitter, amount);
            ...
            _safeTransferReward(task.rewardAsset, taskSubmissions[earliestIdx].submitter, payoutPool);
    ...
    function _safeTransferReward(address asset, address to, uint256 amount) private {
        if (asset == address(0)) {
    ...
    function _transferRewardAndEmit(uint256 taskId, address asset, address to, uint256 amount, Escrow storage escrow) private {
        _safeTransferReward(asset, to, amount);
        escrow.releasedAmount += amount;
        emit RewardReleased(taskId, to, amount);
    }

    function _transferRewardToTreasury(uint256 taskId, address asset, address treasury, uint256 amount, Escrow storage escrow) private {
        _safeTransferReward(asset, treasury, amount);
        ...
        if (splitEqual) {
            ...
            for (uint256 i = 0; i < winnerCount; i++) {
                uint256 amount = (i == winnerCount - 1)
                    ...
                    : rewardPerWinner;
                _transferRewardAndEmit(taskId, task.rewardAsset, winnerAddresses[i], amount, escrow);
            }
        } else {
            uint256 weightSum = 0;
            for (uint256 i = 0; i < winnerCount; i++) {
    ...
    function _distributeProportionally(
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            _transferRewardAndEmit(taskId, asset, recipients[i], amount, escrow);
            distributed += amount;
        }
    }
}
